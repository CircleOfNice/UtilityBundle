...
trait Interfaces {
	...
	protected function mockReflectable($className) {
		$mock = $this->getMock("Ci\UtilityBundle\Interfaces\Reflectable");
	 ...
	 * @return	Ci\UtilityBundle\Interfaces\Printable
	 */
	protected function mockPrintable($toString) {
		$mock = $this->getMockBuilder("Ci\UtilityBundle\Interfaces\Printable")
					 ->getMock();
	 ...
	 * @return	Ci\UtilityBundle\Interfaces\DeepPrintable
	 */
	protected function mockDeepPrintable($toStringFlat, $toStringDeep) {
		$mock = $this->getMockBuilder("Ci\UtilityBundle\Interfaces\DeepPrintable")
		             ->getMock();
	 ...
	 * @param	string	$last
	 * @param	array	$values
	 * @return	Ci\UtilityBundle\Interfaces\Vector
	 */
	protected function mockVector($last = "", array $values = array()) {
		$mock = $this->getMockBuilder("Ci\UtilityBundle\Interfaces\Vector")
		             ->getMock();
	...
	/**
	 * 
	 * @param	Ci\AnnotatableBundle\Interfaces\Vector	$vector
	 * @param	mixed									$value
	 * @return	Ci\UtilityBundle\Interfaces\Branchable
	 */
	public function mockBranchable(Vector $vector, $value = "", $throwsException = false, array $nodes = array()) {
		return $this->mockBranchableFromExistingMock($this->getMock("Ci\UtilityBundle\Interfaces\Branchable"), $vector, $value, $throwsException, $nodes);
	}
